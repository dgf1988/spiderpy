...
class Url(object):
    ...
    def protocol(self, protocol):
        ...
        if protocol not in Url.AcceptProtocol:
            raise ValueError
    ...
    def md5(self):
        return md5(self.str().encode())

    def httpget(self, charset='utf-8', timeout=100):
        response = get(self.str(), timeout=timeout)
...
class UrlBuilder(object):
    ...
    def __eq__(self, other):
        if isinstance(other, UrlBuilder):
            return self.dict() == other.dict()
        return self.str() == str(other)
